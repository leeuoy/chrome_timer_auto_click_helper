(function () {
  ...
  /**
   * CSS 选择器生成器
   * 实现三层策略生成唯一 CSS 选择器：ID 优先 → class 组合 → 相对路径 → 完整路径
   * 参考 chrome_content 插件实现
   * 新增：getShortestSelector 生成最短唯一选择器
   */

  /**
   * 验证选择器在文档中是否只匹配一个元素
   * @param {string} selector - CSS 选择器
   * @returns {boolean} 是否唯一匹配
   */
   ...
   * @returns {string} 选择器片段，如 div.class1.class2:nth-of-type(2)
   */
  function buildNodeSegment(el) {
    const tag = el.tagName.toLowerCase();
    if (tag === 'html' || tag === 'body') return tag;

    let segment = tag;

    /* 附加 class（去除包含 : 的 class，避免与伪类冲突） */
    if (el.id) {
      return `#${CSS.escape(el.id)}`;
    }

    const validClasses = [...el.classList].filter(c => !c.includes(':'));
    if (validClasses.length > 0) {
      segment += '.' + validClasses.map(c => CSS.escape(c)).join('.');
    }

    /* 添加 :nth-of-type 以确保同类型元素中的唯一性 */
  ...
  /**
   * 尝试通过 class 组合生成唯一选择器
   * @param {Element} el - 目标元素
   * @returns {string|null} 唯一选择器或 null
   */
  function tryClassSelector(el) {
    const validClasses = [...el.classList].filter(c => !c.includes(':'));
    if (validClasses.length === 0) return null;

    const selector = el.tagName.toLowerCase() + '.' + validClasses.map(c => CSS.escape(c)).join('.');
    return verifySelector(selector) ? selector : null;
  }

  /**
   * 尝试通过相对路径生成选择器（向上找最近有 ID 的祖先）
   * @param {Element} el - 目标元素
   * @returns {string|null} 相对路径选择器或 null
   */
  function tryRelativePath(el) {
    let current = el.parentElement;
    const segments = [buildNodeSegment(el)];
    ...
    while (current && current !== document.documentElement && depth < 10) {
      if (current.id) {
        /* 找到有 ID 的祖先，构建相对路径 */
        const path = `#${CSS.escape(current.id)} > ${segments.reverse().join(' > ')}`;
        return verifySelector(path) ? path : null;
      }
      segments.push(buildNodeSegment(current));
  ...
  function buildFullPath(el) {
    ...
    while (current && current !== document) {
      segments.unshift(buildNodeSegment(current));
      current = current.parentElement;
    }
  ...
  /**
   * 为指定元素生成最短的唯一 CSS 选择器
   * 按长度从短到长尝试候选，返回第一个唯一匹配的选择器
   * @param {Element} el - 目标 DOM 元素
   * @returns {string} 最短的唯一 CSS 选择器
   */
  function getShortestSelector(el) {
    ...
    const nthIndex = siblings.length > 1 ? siblings.indexOf(el) + 1 : null;
    ...
    /* 2. tag + class（常用且短） */
    if (validClasses.length > 0) {
      candidates.push(tag + '.' + validClasses.map(c => CSS.escape(c)).join('.'));
    }

    /* 3. tag + class + nth-of-type */
    if (validClasses.length > 0 && nthIndex) {
      candidates.push(tag + '.' + validClasses.map(c => CSS.escape(c)).join('.') + `:nth-of-type(${nthIndex})`);
    }

    /* 4. 纯 tag（如果页面上只有一个该标签） */
    candidates.push(tag);

    /* 5. tag + nth-of-type */
    if (nthIndex) {
      candidates.push(`${tag}:nth-of-type(${nthIndex})`);
    }

    /* 6. 逐步添加上级祖先（从最近到最远） */
    let current = el.parentElement;
    let childSelector = buildNodeSegment(el);
    while (current && current !== document.documentElement) {
      const ancestorSegment = buildNodeSegment(current);
      childSelector = ancestorSegment + ' > ' + childSelector;
      candidates.push(childSelector);
      current = current.parentElement;
    }

    /* 验证每个候选，返回第一个唯一的 */
    for (const selector of candidates) {
      if (verifySelector(selector)) return selector;
    }
  ...
  /**
   * 为指定元素生成唯一的 CSS 选择器（完整路径策略）
   * 三层策略：ID → class 组合 → 相对路径 → 完整路径
   * @param {Element} el - 目标 DOM 元素
   * @returns {string} 唯一的 CSS 选择器
   */
  function getUniqueSelector(el) {
    /* 第一层：优先使用 ID */
    if (el.id) {
      const idSelector = `#${CSS.escape(el.id)}`;
      if (verifySelector(idSelector)) return idSelector;
    }
    ...
    /* 第三层：相对路径 */
    const relPath = tryRelativePath(el);
  ...
  /**
   * 创建任务面板模块
   * 在页面 DOM 中注入一个浮动的创建任务确认面板
   * 用于选择元素后直接在页面内配置并创建任务，避免 popup 关闭导致的体验问题
   */
  ...
  let panel$1 = null;
  /** 任务创建完成后的回调函数 */
  let onTaskCreatedCallback$1 = null;

  /**
   * 创建面板 DOM 元素并添加到页面
   * @param {object} pickData - 选中的元素数据
   * @param {string} pickData.selector - CSS 选择器
   * @param {string} pickData.tagName - 标签名
   * @param {string} pickData.text - 元素文本内容
   * @param {string} domain - 当前域名
   */
  function createPanel$1(pickData, domain) {
    /* 半透明遮罩层 */
    overlay = document.createElement('div');
    overlay.id = 'chrome-timer-create-overlay';
    ...
    panel$1.id = 'chrome-timer-create-panel';
  ...
  `;
    ...
    cronInput.placeholder = '*/3 * * * * *';
    applyInputStyle(cronInput);
    cronGroup.appendChild(cronInput);
    panel$1.appendChild(cronGroup);
    ...
    toggleWrap.style.cssText = 'position:relative;display:inline-block;width:40px;height:22px;cursor:pointer;';
    ...
    background-color:#4285f4;border-radius:22px;transition:0.2s;
  `;
  ...
  `;
    ...
    createBtn.addEventListener('click', async () => {
      ...
      /* 构建脚本内容：替换占位符 */
      const script = DEFAULT_SCRIPT.replace('{{SELECTOR}}', pickData.selector);
      ...
      const tasks = await getDomainTasks(domain);
      tasks.push(newTask);
      await saveDomainTasks(domain, tasks);

      /* 通知回调（更新调度器） */
      if (onTaskCreatedCallback$1) {
        onTaskCreatedCallback$1(tasks);
      }
    ...
    panel$1.appendChild(btnRow);

    /* 点击遮罩层关闭 */
    overlay.addEventListener('click', (e) => {
      if (e.target === overlay) closePanel$1();
    });
  ...
  /**
   * 创建表单组（label + input 容器）
   * @param {string} labelText - 标签文字
   * @returns {HTMLDivElement} 表单组 DOM
   */
  ...
  /**
   * 应用输入框样式
   * @param {HTMLInputElement} el - 输入框元素
   */
   ...
   */
  function applyPrimaryBtnStyle(btn) {
    ...
    btn.addEventListener('mouseenter', () => {
      btn.style.backgroundColor = '#3367d6';
    });
   ...
   * @param {HTMLButtonElement} btn - 按钮元素
   */
  function applySecondaryBtnStyle(btn) {
    Object.assign(btn.style, {
      padding: '8px 20px',
      border: '1px solid #ddd',
      borderRadius: '6px',
   ...
   * @param {string} pickData.selector - CSS 选择器
   * @param {string} pickData.tagName - 标签名
   * @param {string} pickData.text - 元素文本
   * @param {string} domain - 当前域名
   * @param {function} onCreated - 任务创建成功后的回调，参数为新的任务列表
   */
  function showCreateTaskPanel(pickData, domain, onCreated) {
    /* 如果已有面板，先关闭 */
    if (overlay) closePanel$1();

    onTaskCreatedCallback$1 = onCreated || null;
    createPanel$1(pickData, domain);
  }

  /**
   * 元素选择器模块
   * 在页面上实现鼠标悬停高亮、点击选取元素的功能
   * 悬停时实时显示当前元素的选择器表达式气泡
   * 支持精简/完整选择器切换
   */
  ...
  /** 是否处于选择模式 */
  let picking = false;
  /** 是否使用完整选择器（由外部 popup 传入） */
  let useFullSelector$1 = false;
  /** 当前页面域名 */
  let currentDomain$1 = '';
  /** 任务创建后的回调（用于更新调度器） */
  let onTaskCreatedCallback = null;
  /** 事件处理函数引用（用于移除监听） */
  const handlers = {};
  /** 节流：上一次生成选择器的时间戳 */
  let lastSelectorTime = 0;
  /** 节流间隔（毫秒） */
  const THROTTLE_MS = 50;

  /**
   * 创建高亮框、选择器气泡和遮罩层 DOM 元素
   */
  function createUI() {
    /* 半透明遮罩层 */
    captureLayer = document.createElement('div');
    captureLayer.id = 'chrome-timer-capture-layer';
    ...
    highlightBox.id = 'chrome-timer-highlight';
    ...
    tooltip.id = 'chrome-timer-tooltip';
   ...
   */
  function updateTooltipContent(selector) {
    if (tooltip) {
  ...
  function updateTooltipPosition(mouseX, mouseY) {
    ...
    let top = mouseY - ttH - gap;
    let left = mouseX + 12;

    if (top < 4) {
      top = mouseY + gap + 12;
    }
    if (left + ttW > window.innerWidth - 4) {
  ...
  /**
   * 开始元素选择模式
   * @param {boolean} fullSelector - 是否使用完整选择器（默认 false 表示精简）
   * @param {string} domain - 当前页面域名
   * @param {function} onTaskCreated - 任务创建成功后的回调
   * @param {function} onPicked - 元素被选中后的回调（用于编辑器模式，直接传入选择器数据）
   */
  function startPicker(fullSelector = false, domain = '', onTaskCreated = null, onPicked = null) {
    if (picking) return;
    picking = true;
    useFullSelector$1 = fullSelector;
    currentDomain$1 = domain;
    onTaskCreatedCallback = onTaskCreated;
    createUI();
    ...
    handlers.mousemove = (e) => {
      ...
      if (el && el !== highlightBox && el !== captureLayer && !el.closest('#chrome-timer-tooltip')) {
        updateHighlight(el);

        const now = Date.now();
        if (now - lastSelectorTime >= THROTTLE_MS) {
          lastSelectorTime = now;
          const selector = useFullSelector$1
            ? getUniqueSelector(el)
            : getShortestSelector(el);
          updateTooltipContent(selector);
        }
    ...
    handlers.click = (e) => {
      /* 忽略气泡内部的点击 */
      if (e.target.closest('#chrome-timer-tooltip')) return;
      e.preventDefault();
      e.stopPropagation();
      hideHighlight();
      const el = document.elementFromPoint(e.clientX, e.clientY);
      if (el && el !== highlightBox && el !== captureLayer) {
        const selector = useFullSelector$1
          ? getUniqueSelector(el)
          : getShortestSelector(el);
        const pickData = {
        ...
        /* 如果传了 onPicked 回调（编辑器模式），直接传回选择器数据 */
        if (onPicked) {
          onPicked(pickData);
        } else {
          /* 在页面内显示创建任务面板，替代 popup 确认框 */
          showCreateTaskPanel(pickData, currentDomain$1, onTaskCreatedCallback);
        }
      }
    };
  ...
  function stopPicker() {
    if (!picking) return;
    picking = false;
    if (handlers.mousemove) document.removeEventListener('mousemove', handlers.mousemove, true);
  ...
  /**
   * 脚本执行器模块
   * 通过 background service worker 使用 chrome.scripting.executeScript 在页面 MAIN world 执行脚本
   * 绕过页面 CSP 限制，错误不会中断调度循环
   */

  /** 上下文失效时的回调（由 cron-scheduler.js 注册） */
  let onContextInvalidatedCallback = null;

  /**
   * 注册扩展上下文失效时的回调
   * 由 cron-scheduler.js 在启动时调用，打破循环依赖
   * @param {function} callback - 清理调度器的回调函数
   */
  function registerContextInvalidatedCallback(callback) {
    onContextInvalidatedCallback = callback;
  }
  ...
  /**
   * 降级执行：通过创建 <script> 标签直接在页面中执行脚本
   * @param {string} code - 用户自定义 JavaScript 代码
   */
  function executeScriptFallback(code) {
    try {
      ...
      console.error('[Chrome Timer] 降级脚本执行也失败:', err.message);
    }
  }

  /**
   * 在页面 MAIN world 中执行用户脚本
   * @param {string} code - 用户自定义 JavaScript 代码
   */
  function executeScript(code) {
    if (!isExtensionContextValid()) {
      console.warn('[Chrome Timer] 扩展上下文已失效，停止调度并尝试降级执行');
      onContextInvalidatedCallback?.();
      executeScriptFallback(code);
      return;
    }
    ...
    chrome.runtime.sendMessage({
      ...
      if (msg.includes('Extension context invalidated')) {
        ...
        console.error('[Chrome Timer] 脚本执行请求失败:', msg);
      }
    });
  }
  ...
  /**
   * 编辑任务面板模块
   * 在页面 DOM 中注入可拖拽、可最大化、可最小化、可调整大小的浮动编辑窗口
   * CodeMirror 通过动态 import() 按需加载，不打包进 content.js
   */
  ...
  function onResizeMove(e) {
    ...
    /* 限制不超过视口 */
    panel.style.width = Math.min(newW, vw - parseInt(panel.style.left || 0)) + 'px';
    panel.style.height = Math.min(newH, vh - parseInt(panel.style.top || 0)) + 'px';
  }
  ...
  function toggleMaximize(maxBtn) {
    if (isMaximized) {
      ...
      savedRect = {
        ...
        top: panel.style.top || '',
        width: panel.style.width || '80vw',
        height: panel.style.height || '80vh',
      };
      panel.style.left = '0';
      panel.style.top = '0';
  ...
  function toggleMinimize(minBtn, bodyWrapper) {
    /* 查询标题栏内的元素，最小化时隐藏以腾出按钮空间 */
    const nameInput = panel.querySelector('#chrome-timer-name-input');
    const cronInput = panel.querySelector('#chrome-timer-cron-input');
    const titleIcon = panel.querySelector('#chrome-timer-title-icon');

    if (isMinimized) {
      /* 恢复 */
      bodyWrapper.style.display = 'flex';
      if (savedRect) {
        panel.style.height = savedRect.height || '80vh';
        panel.style.width = savedRect.width || '80vw';
      } else {
  ...
  function createToolBtn(iconSvg, title, onClick) {
    ...
    Object.assign(btn.style, {
      ...
      color: '#a6adc8', fontSize: '12px',
      transition: 'all 0.15s',
    });
    btn.addEventListener('mousedown', (e) => e.stopPropagation());
    btn.addEventListener('mouseenter', () => { btn.style.backgroundColor = 'rgba(255,255,255,0.06)'; btn.style.borderColor = '#45475a'; });
    btn.addEventListener('mouseleave', () => { btn.style.backgroundColor = 'transparent'; btn.style.borderColor = 'transparent'; });
    btn.addEventListener('click', onClick);
    return btn;
  }

  /**
   * 创建一个带图标的工具栏按钮（支持激活状态）
   * @param {string} iconSvg
   * @param {string} label
   * @param {string} title
   * @param {function} onClick
   * @param {string} activeColor - 激活状态颜色
   */
  function createToggleBtn(iconSvg, label, title, onClick, activeColor = '#89b4fa') {
    let active = false;
    const btn = document.createElement('button');
    btn.innerHTML = iconSvg;
    btn.title = title;
    ...
    btn.addEventListener('mousedown', (e) => e.stopPropagation());
    btn.addEventListener('mouseenter', () => { btn.style.backgroundColor = 'rgba(255,255,255,0.06)'; btn.style.borderColor = '#45475a'; });
    btn.addEventListener('mouseleave', () => { if (!active) { btn.style.backgroundColor = 'transparent'; btn.style.borderColor = 'transparent'; } });
    btn.addEventListener('click', () => {
      active = !active;
      if (active) {
        btn.style.color = activeColor;
        btn.style.borderColor = activeColor;
        btn.style.backgroundColor = 'rgba(137,180,250,0.1)';
      } else {
  ...
  /**
   * 启动元素选择器，选择完成后将选择器插入编辑器光标位置
   * @param {boolean} full - 是否使用完整选择器
   */
  function startElementPicker(full) {
    /* 最小化编辑器面板，方便选择页面元素 */
    if (!isMinimized && panel) {
      const minBtn = panel.querySelector('#chrome-timer-min-btn');
      const bodyWrapper = panel.querySelector('#chrome-timer-body-wrapper');
      if (minBtn && bodyWrapper) {
        toggleMinimize(minBtn, bodyWrapper);
      }
    }

    startPicker(full, '', null, (pickData) => {
      /* 选择完成后恢复编辑器 */
      if (isMinimized && panel) {
        const minBtn = panel.querySelector('#chrome-timer-min-btn');
        const bodyWrapper = panel.querySelector('#chrome-timer-body-wrapper');
      ...
      /* 将选择器包裹为 document.querySelector 插入编辑器光标位置 */
      if (editorInstance && pickData.selector) {
        const textToInsert = `document.querySelector("${pickData.selector}")`;
        editorInstance.insertAtCursor(textToInsert);
      }
    });
  }
   ...
   */
  async function createPanel(task, domain, onSaved) {
    editingTaskId = task.id;
    onTaskSavedCallback = onSaved;
    isMaximized = false;
    isMinimized = false;
    savedRect = null;
    useFullSelector = false;

    /* 面板主体（直接添加到页面，无遮罩） */
    panel = document.createElement('div');
    panel.id = 'chrome-timer-edit-panel';
    const vw = window.innerWidth, vh = window.innerHeight;
    const pw = Math.min(vw * 0.8, 1000), ph = vh * 0.8;
    Object.assign(panel.style, {
      ...
      overflow: 'hidden',
      color: '#cdd6f4', fontSize: '14px',
      zIndex: '2147483647',
      animation: 'chromeTimerEditPanelIn 0.2s ease-out',
      minWidth: '320px',
      minHeight: '120px',
    });
    ...
    styleEl.id = 'chrome-timer-edit-panel-style';
    styleEl.textContent = `
    @keyframes chromeTimerEditPanelIn {
  ...
  `;
    ...
    /* 标题栏任意位置（按钮和输入框除外）均可拖拽 */
    header.addEventListener('mousedown', onDragStart);

    /* 左侧：图标 + 名称 + Cron */
    const titleRow = document.createElement('div');
    Object.assign(titleRow.style, { display: 'flex', alignItems: 'center', gap: '10px', flex: '1' });

    /* 拖拽图标（纯装饰，提示可拖拽） */
    const dragIcon = document.createElement('div');
    dragIcon.innerHTML = '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="#6c7086" stroke-width="2"><path d="M8 6h8M8 12h8M8 18h8"/></svg>';
    ...
    titleIcon.id = 'chrome-timer-title-icon';
    titleIcon.textContent = '✏️';
    Object.assign(titleIcon.style, { fontSize: '16px', cursor: 'default' });
    titleRow.appendChild(titleIcon);

    const nameInput = document.createElement('input');
    nameInput.id = 'chrome-timer-name-input';
    nameInput.type = 'text';
    nameInput.value = task.name;
    Object.assign(nameInput.style, {
      backgroundColor: 'transparent', border: '1px solid transparent',
      borderRadius: '6px', color: '#cdd6f4', fontSize: '15px', fontWeight: '600',
      padding: '2px 8px', outline: 'none', flex: '1', maxWidth: '260px', minWidth: '80px',
      transition: 'border-color 0.2s, background-color 0.2s',
      cursor: 'auto',
    });
    nameInput.addEventListener('mousedown', (e) => e.stopPropagation());
    nameInput.addEventListener('focus', () => { nameInput.style.borderColor = '#89b4fa'; nameInput.style.backgroundColor = 'rgba(137,180,250,0.06)'; });
    nameInput.addEventListener('blur', () => { nameInput.style.borderColor = 'transparent'; nameInput.style.backgroundColor = 'transparent'; });
    titleRow.appendChild(nameInput);

    const cronInput = document.createElement('input');
    cronInput.id = 'chrome-timer-cron-input';
    cronInput.type = 'text';
    cronInput.value = task.cronExpression;
    cronInput.placeholder = '*/3 * * * * *';
    Object.assign(cronInput.style, {
      backgroundColor: 'transparent', border: '1px solid transparent',
      borderRadius: '6px', color: '#a6adc8', fontSize: '13px',
      fontFamily: '"JetBrains Mono", "Consolas", monospace',
      padding: '2px 8px', outline: 'none', width: '170px', letterSpacing: '0.5px',
      transition: 'border-color 0.2s, background-color 0.2s',
      cursor: 'auto',
    });
    cronInput.addEventListener('mousedown', (e) => e.stopPropagation());
    cronInput.addEventListener('focus', () => { cronInput.style.borderColor = '#89b4fa'; cronInput.style.backgroundColor = 'rgba(137,180,250,0.06)'; });
    cronInput.addEventListener('blur', () => { cronInput.style.borderColor = 'transparent'; cronInput.style.backgroundColor = 'transparent'; });
    titleRow.appendChild(cronInput);
    header.appendChild(titleRow);

    /* 右侧按钮组 */
    const btnGroup = document.createElement('div');
    Object.assign(btnGroup.style, { display: 'flex', gap: '4px', cursor: 'auto' });
    ...
    minBtn.addEventListener('mouseenter', () => { minBtn.style.backgroundColor = 'rgba(255,255,255,0.08)'; });
    ...
    maxBtn.addEventListener('mouseenter', () => { maxBtn.style.backgroundColor = 'rgba(255,255,255,0.08)'; });
    maxBtn.addEventListener('mouseleave', () => { maxBtn.style.backgroundColor = 'transparent'; });
    maxBtn.addEventListener('click', () => toggleMaximize(maxBtn));
    ...
    closeBtn.addEventListener('mouseenter', () => { closeBtn.style.backgroundColor = 'rgba(235,87,87,0.2)'; closeBtn.querySelector('svg').setAttribute('stroke', '#eb5757'); });
    closeBtn.addEventListener('mouseleave', () => { closeBtn.style.backgroundColor = 'transparent'; closeBtn.querySelector('svg').setAttribute('stroke', '#6c7086'); });
    ...
    bodyWrapper.id = 'chrome-timer-body-wrapper';
    ...
    Object.assign(sep.style, { width: '1px', height: '18px', backgroundColor: '#313244', margin: '0 4px' });
    ...
    toolbar.appendChild(fullSelectorBtn);
    toolbar.appendChild(sep);
    toolbar.appendChild(formatBtn);
    toolbar.appendChild(wrapBtn);
    toolbar.appendChild(tipLabel);
    bodyWrapper.appendChild(toolbar);
    ...
    bodyWrapper.appendChild(editorContainer);
    ...
    runBtn.addEventListener('mouseenter', () => { runBtn.style.backgroundColor = 'rgba(166,227,161,0.1)'; });
    runBtn.addEventListener('mouseleave', () => { runBtn.style.backgroundColor = 'transparent'; });
    ...
    saveBtn.addEventListener('mouseenter', () => { saveBtn.style.backgroundColor = '#74c7ec'; });
    saveBtn.addEventListener('mouseleave', () => { saveBtn.style.backgroundColor = '#89b4fa'; });
    saveBtn.addEventListener('click', () => doSave(nameInput, cronInput));

    footerBtns.appendChild(runBtn);
    ...
    resizeHandle.id = 'chrome-timer-resize-handle';
    Object.assign(resizeHandle.style, {
      ...
      cursor: 'nwse-resize',
      zIndex: '10',
      background: 'linear-gradient(135deg, transparent 50%, rgba(166,173,200,0.3) 50%)',
    ...
    /* 绑定最小化按钮事件 */
    minBtn.addEventListener('click', () => toggleMinimize(minBtn, bodyWrapper));
    ...
    /* ========== 动态加载 CodeMirror 编辑器 ========== */
    try {
      const editorUrl = chrome.runtime.getURL('editor-module.js');
      const module = await import(editorUrl);
      /* 移除加载提示 */
      loadingEl.remove();
      /* 创建编辑器 */
      const scriptContent = task.script || '';
      console.log('[Chrome Timer] 编辑器加载，脚本长度:', scriptContent.length, '任务ID:', task.id);
      editorInstance = module.createEditor(editorContainer, {
    ...
    /* ========== 保存逻辑 ========== */
    async function doSave(nameEl, cronEl) {
      const name = nameEl.value.trim() || task.name;
      const cronExpression = cronEl.value.trim() || task.cronExpression;
      const script = editorInstance ? editorInstance.getValue() : (task.script || '');
      ...
      if (target) {
        ...
        await saveDomainTasks(domain, tasks);
        if (onTaskSavedCallback) {
          onTaskSavedCallback(tasks);
        }
      }
      closePanel();
    }
  ...
  function closePanel() {
    ...
    const styleEl = document.getElementById('chrome-timer-edit-panel-style');
    if (styleEl) styleEl.remove();

    editingTaskId = null;
    onTaskSavedCallback = null;
    isMaximized = false;
    isMinimized = false;
    savedRect = null;
    useFullSelector = false;
   ...
   * @param {function} onSaved - 保存成功回调
   */
  function showEditTaskPanel(task, domain, onSaved) {
    if (panel) closePanel();
    createPanel(task, domain, onSaved);
  }
  ...
  function requireCronExpressionParser () {
  	 ...
  	 */
  	let CronExpressionParser$1 = class CronExpressionParser {
  	    /**
  ...
  function requireCronFileParser () {
  	...
  	let CronFileParser$1 = class CronFileParser {
  	    ...
  	    static async parseFile(filePath) {
  	        const { readFile } = await Promise.resolve().then(() => __importStar(require$$2));
  ...
  /**
   * Cron 调度引擎
   * 在 content script 中以 1 秒为精度检查和触发任务执行
   * 支持 6 字段 Cron 表达式，带解析缓存和页面可见性感知
   */
  ...
  /** 当前域名下的任务列表（内存副本） */
  let tasks = [];
  /** Cron 解析结果缓存：Map<taskId, { expression, interval }> */
  const cache = new Map();
  /** 上一次触发的秒级时间戳，防止同一秒重复触发 */
  const lastTriggered = new Map();

  /**
   * 解析 Cron 表达式（带缓存）
   * @param {string} taskId - 任务 ID
   * @param {string} expression - 6 字段 Cron 表达式
   * @returns {object|null} cron-parser 解析后的 interval 对象，解析失败返回 null
   */
  function parseCron(taskId, expression) {
    const cached = cache.get(taskId);
    if (cached && cached.expression === expression) {
      return cached.interval;
    }
    try {
      const interval = distExports.CronExpressionParser.parse(expression, { throwOnParseError: true });
      cache.set(taskId, { expression, interval });
      return interval;
    } catch (err) {
      console.error(`[Chrome Timer] Cron 表达式解析失败 (${expression}):`, err.message);
      cache.delete(taskId);
      return null;
    }
  }
  ...
  /**
   * 检查当前时间是否匹配 Cron 表达式的所有字段
   * 通过直接访问 interval.fields.*.values 数组进行匹配
   * @param {object} interval - cron-parser 解析后的 interval 对象
   * @param {Date} now - 当前时间
   * @returns {boolean} 是否匹配
   */
  function matchCron(interval, now) {
    try {
      return interval.fields.second.values.includes(now.getSeconds()) &&
             interval.fields.minute.values.includes(now.getMinutes()) &&
             interval.fields.hour.values.includes(now.getHours()) &&
             interval.fields.dayOfMonth.values.includes(now.getDate()) &&
             interval.fields.month.values.includes(now.getMonth() + 1) && /* month 0-11 → 1-12 */
             interval.fields.dayOfWeek.values.includes(now.getDay());
    } catch (err) {
      console.error('[Chrome Timer] Cron 匹配异常:', err.message);
      return false;
    }
  }

  /**
   * 获取当前时间到秒级精度的时间戳（用于去重）
  ...
  function tick() {
    ...
    for (const task of tasks) {
      ...
      /* 防止同一秒内重复触发 */
      if (lastTriggered.get(task.id) === currentSecond) continue;

      const interval = parseCron(task.id, task.cronExpression);
      if (!interval) continue;

      /* 检查当前时间是否匹配 Cron 表达式 */
      if (matchCron(interval, now)) {
        lastTriggered.set(task.id, currentSecond);
        console.log(`[Chrome Timer] 执行任务: ${task.name}`);
  ...
  /**
   * 更新内存中的任务列表
   * @param {Array} newTasks - 最新的任务列表
   */
  function updateTasks(newTasks) {
    tasks = newTasks;
    console.log(`[Chrome Timer] 任务列表已更新，共 ${tasks.length} 个任务，启用 ${tasks.filter(t => t.enabled).length} 个`);
  }

  /**
   * 启动调度器
   * @param {Array} initialTasks - 初始任务列表
   */
  function startScheduler(initialTasks) {
    tasks = initialTasks || [];
    console.log(`[Chrome Timer] 调度器启动，任务数: ${tasks.length}`);
    /* 注册扩展上下文失效时的清理回调 */
    registerContextInvalidatedCallback(stopScheduler);
    startInterval();

    /* 页面可见性感知：后台暂停，前台恢复 */
    document.addEventListener('visibilitychange', () => {
      if (document.hidden) {
        console.log('[Chrome Timer] 页面隐藏，暂停调度');
        stopInterval();
      } else {
        console.log('[Chrome Timer] 页面显示，恢复调度');
        /* 恢复时清除秒级触发记录 */
        lastTriggered.clear();
        startInterval();
      }
  ...
  /**
   * Content Script 入口
   * 负责初始化调度引擎、注册消息监听、处理与 popup 的通信
   * 在页面 document_idle 时自动注入
   */
  ...
  function setupMessageListeners() {
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
      switch (message.action) {
        /* 返回当前域名 */
        case MSG.GET_CURRENT_DOMAIN:
          sendResponse({ domain: currentDomain });
          return false;

        /* 返回当前域名的任务列表 */
        case MSG.GET_DOMAIN_TASKS:
          getDomainTasks(currentDomain).then(tasks => {
            sendResponse({ tasks });
          });
          return true; /* 异步响应 */
        ...
        case MSG.START_PICKER: {
          const useFull = message.data?.useFullSelector === true;
          startPicker(useFull, currentDomain, (tasks) => {
            /* 任务创建成功后更新调度器 */
            updateTasks(tasks);
          });
        ...
        /* popup 切换任务启用/禁用 */
        case MSG.TOGGLE_TASK: {
          const { taskId, enabled } = message.data;
          /* 直接从 storage 重新读取最新数据 */
          (async () => {
            const domainTasks = await getDomainTasks(currentDomain);
            const task = domainTasks.find(t => t.id === taskId);
            if (task) {
              task.enabled = enabled;
              task.updatedAt = Date.now();
              await saveDomainTasks(currentDomain, domainTasks);
              updateTasks(domainTasks);
            }
            sendResponse({ success: true });
          })();
          return true; /* 异步响应 */
        ...
        /* popup 删除任务 */
        case MSG.DELETE_TASK: {
          const { taskId: delTaskId } = message.data;
          (async () => {
            await deleteDomainTask(currentDomain, delTaskId);
            const remainingTasks = await getDomainTasks(currentDomain);
        ...
        case MSG.OPEN_EDITOR: {
          ...
          showEditTaskPanel(task, currentDomain, (tasks) => {
            /* 保存成功后更新调度器 */
            updateTasks(tasks);
          });
          sendResponse({ success: true });
        ...
        /* popup 请求立即执行一次任务脚本（直接执行传入的代码） */
        case MSG.RUN_TASK_NOW: {
          const { code } = message.data;
          if (code) {
            console.log('[Chrome Timer] 手动执行脚本');
            executeScript(code);
            sendResponse({ success: true });
          } else {
